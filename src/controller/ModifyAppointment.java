            ...
            if(!appError){
                ...
                if(dbAppointment.checkAppStart(appStart) && dbAppointment.checkAppEnd(appEnd)){
                    if (appStart.toLocalTime().isBefore(appEnd.toLocalTime())) {
                        if (!dbAppointment.checkModifyAppOverlap(customerId, startDate, endDate, startTime, endTime, appId)) {
    ...
    public void errorMessage(int errorCode){
        switch (errorCode){
            ...
            case 7 -> {
                ...
                alert.setContentText("There is an overlapping appointment with Customer. Please select a different time slot");
                alert.showAndWait();
            }
            case 8 -> {
                Alert alert = new Alert(Alert.AlertType.ERROR);
