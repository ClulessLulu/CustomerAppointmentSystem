...
public class DBAppointmentImpl implements DBAppointment {
    ...
    @Override
    public boolean checkNewAppOverlap(int customerId, LocalDate overlapStartDate, LocalDate overlapEndDate, LocalTime overlapStartTime, LocalTime overlapEndTime) {
     DBAppointment dbAppointment = new DBAppointmentImpl();
     ObservableList<Appointment> cxOverlapCheck = dbAppointment.getAppByCxId(customerId);
    ...
    @Override
    public boolean checkModifyAppOverlap(int customerId, LocalDate overlapStartDate, LocalDate overlapEndDate, LocalTime overlapStartTime, LocalTime overlapEndTime, int appId) {
        DBAppointment dbAppointment = new DBAppointmentImpl();
        ObservableList<Appointment> modOverlapCheck = dbAppointment.getAppByCxId(customerId);
        boolean modOverlap = false;
        for (Appointment appointment : modOverlapCheck){
            if ((appointment.getAppId() == appId) && (overlapStartTime.equals(appointment.getStartTime()) && (overlapEndTime.equals(appointment.getEndTime())))){
                break;
            }
            else {
                checkNewAppOverlap(customerId, overlapStartDate, overlapEndDate, overlapStartTime, overlapEndTime);
            }
        }
        return modOverlap;
    }
